...
def main() -> int:
    ...
    try:
        ...
        if live_stream and not live_snap:
            ...
            print("VERDICT: no real-time data through the API at all; delayed")
            print("  (type 3) works. The subscription is not API-enabled.")
            print("  Options: run at reqMarketDataType(3) and accept ~15-minute")
            print("  data (useless for a 5-minute intraday strategy), or enable")
            print("  API access for the existing subscription in Client Portal.")
        else:
